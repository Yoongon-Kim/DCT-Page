...
def inf_llm_forward(
    ...
    *args, **kwargs
    ...
    # Construction-time triton guard: the triton backend builds its mask via
    # constexpr SLIDING_WINDOW only, with no second output_window path. Reject
    # narrowed-output-window configs at patch time (NOT first-forward) so the
    # error surfaces during model load.
