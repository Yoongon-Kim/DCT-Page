...
class ContextManager:
    def __init__(self,
        ...
        # n_recent decouples the local-attention output window from the scoring
        # horizon. None → use n_local (back-compat). Must satisfy n_recent <= n_local
        # because the output-mask is chained on top of the wide score-mask in
        # torch_impl.finalize() (exploits output_mask ⊆ mask inclusion invariant).
        self.n_recent = n_local if n_recent is None else n_recent
        assert self.n_recent <= self.n_local, (
            f"n_recent ({self.n_recent}) must be <= n_local ({self.n_local})"
        )
    ...
    def _append(
        ...
        # calc global result
        # global path: output_window intentionally omitted — the narrow window
        # only applies to the local sliding-window output, not the complement path.
        attn.append(
