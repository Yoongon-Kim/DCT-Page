...
Two experiments live in this module; pick one with --mode:

    --mode full   (default) multi-selector recall: DCT / Quest / ShadowKV /
                  InfLLM / oracle_max on the DCT-Page sink+recent layout.
    --mode quest  Quest-native geometry (last-page keep, skip layers 0-1),
                  isolating Quest's selection rule from the sink/recent floor.

Internals: the two paths keep separate entry points (run_full / run_quest,
parse_args_full / parse_args_quest) and the Quest path uses QUEST_*-prefixed
metric-key globals so the two metric sets do not collide.
"""
#!/usr/bin/env python3
...
QUEST_FIDELITY_METRIC_KEYS = [
    "output_fidelity_quest",
    "output_fidelity_oracle_max",
]

QUEST_METRIC_KEYS = QUEST_MASS_METRIC_KEYS + QUEST_FIDELITY_METRIC_KEYS
...
class QuestMassRecallRecorder:
    ...
    def __call__(self, payload: dict[str, Any]) -> None:
        ...
        for key, tensor in metrics.items():
            lo, hi = float(tensor.min()), float(tensor.max())
            if key in QUEST_FIDELITY_METRIC_KEYS:
                low_bound, high_bound = -1.0, 1.0
            else:
                low_bound, high_bound = 0.0, 1.0
        ...
        self.records.append({
            ...
            "num_kv_groups": num_kv_groups,
            "H_q": int(H_q),
            **{k: metrics[k].tolist() for k in QUEST_METRIC_KEYS},
        })


def generate_with_mass_traces_quest(
...
# CLI
# ---------------------------------------------------------------------------
def parse_args_quest() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
...
def _aggregate_metric_dicts_quest(dicts: list[dict[str, float]]) -> dict[str, float]:
    if not dicts:
        return {k: 0.0 for k in QUEST_METRIC_KEYS}
    return {k: _mean([d[k] for d in dicts if k in d]) for k in QUEST_METRIC_KEYS}
    ...
    try:
        for task in args.tasks:
            ...
            for sample_idx, sample in enumerate(
                ...
                records, input_len = generate_with_mass_traces_quest(
                ...
                for rec in records:
                    ...
                    for q in range(H_q):
                        flat = {k: rec[k][q] for k in QUEST_METRIC_KEYS}
                        row = {
                            "layer_idx": layer_idx,
                            "decode_step": decode_step,
                            "q_head": q,
                ...
                per_layer_mean = {
                    str(lyr): _aggregate_metric_dicts_quest(bucket)
                    for lyr, bucket in sorted(per_layer_buckets.items())
                }
                ...
                if sample_idx % 5 == 0 or sample_idx == len(samples):
                    o = _aggregate_metric_dicts_quest(task_overall_records)
                    print(
                        f"  [{sample_idx}/{len(samples)}] "
            ...
            per_task_results[task] = {
                "num_samples": len(samples),
                "overall": _aggregate_metric_dicts_quest(task_overall_records),
                "per_layer": {
                    str(lyr): _aggregate_metric_dicts_quest(bucket)
                    for lyr, bucket in sorted(task_per_layer.items())
                },
        ...
        overall_task_means = [r["overall"] for r in per_task_results.values()]
        overall = _aggregate_metric_dicts_quest(overall_task_means)
...
def main() -> None:
    """Dispatch to the full or quest experiment based on a leading --mode flag."""
    import sys as _sys
    ...
    if mode == "quest":
        ...
        raise SystemExit(f"--mode must be 'full' or 'quest', got {mode!r}")
