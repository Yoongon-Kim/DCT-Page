...
from triton_kernels import apply_rope_q_direct, assemble_kv_drop_triton, build_assemble_cache
...
def profiled_dct_page_attention_forward(
    ...
    **kwargs,
    ...
    if cfg.unselected_mode == "drop":
        ...
        # Build cache on first call, reuse on all subsequent calls
        if not hasattr(self, '_assemble_cache'):
            self._assemble_cache = build_assemble_cache(
                paged_k, comp_k, sink_k, recent_k, selected_indices,
                cos_table, self._final_k_buf, self._final_v_buf,
                query_states=query_states if cfg.continuous_rope else None,
            )

        # Compressed mode: fused Triton assemble (Kernel A + B + Q-RoPE) with cached params
        if cfg.continuous_rope:
            final_k, final_v, q_rope_out = assemble_kv_split_triton(
                ...
                cached_params=self._assemble_cache,
            )
