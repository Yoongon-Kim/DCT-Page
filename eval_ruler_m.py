"""
RULER evaluation using MInference's RULER scripts (MInference/experiments/ruler).

A/B-comparison sibling to eval_ruler.py. This script uses MInference's RULER
scripts verbatim — the same setup that historically produced ~50% cwe on
Llama-3.1-8B-Instruct.

Key differences from eval_ruler.py:
  * RULER_DIR → benchmark/eval_ruler_minference/ (MInference's scripts)
  * RULER_DATA_DIR → benchmark/data/ruler_data_minference/ (separate data)
  * Prep uses --model_template_type llama-3 or qwen-3 (baked chat template);
    MInference's prepare.py doesn't support --prepare_for_ns.
  * Inference uses plain tokenizer(sample["input"], return_tensors="pt") with
    no chat-template manipulation — the chat template is already baked into
    `input` at prep time (via MInference's llama-3 or qwen-3 template).
  * Prepared JSONL has {index, input, outputs, length} only — no separate
    answer_prefix field.
  * Qwen3's template pre-closes the thinking block (<think>\n\n</think>\n\n)
    so the answer_prefix continues directly in the final-answer region.

Usage examples:
    # Prepare data + run baseline
    python eval_ruler_m.py --mode baseline \
        --base_model meta-llama/Llama-3.1-8B-Instruct \
        --prepare \
        --output_dir results/results_ruler_m --run_name llama_baseline_m

    # Reuse prepared data
    python eval_ruler_m.py --mode baseline \
        --base_model meta-llama/Llama-3.1-8B-Instruct \
        --output_dir results/results_ruler_m
"""
...
RULER_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "benchmark", "eval_ruler_minference")
RULER_DATA_DIR = Path(os.path.dirname(os.path.abspath(__file__))) / "benchmark" / "data" / "ruler_data_minference"
...
def infer_model_family(base_model: str) -> tuple[str, str]:
    ...
    Template types reference benchmark/eval_ruler_minference/data/template.py:
      ...
      - qwen-3: "<|im_start|>user\\n{task_template}<|im_end|>\\n<|im_start|>assistant\\n<think>\\n\\n</think>\\n\\n"
        (thinking is pre-closed so answer_prefix continues directly in the
        final-answer region rather than being swallowed by a <think> block.)
    """
    s = base_model.lower()
    if "llama" in s:
...
def prepare_data(args):
    """Run eval_ruler_minference/data/prepare.py for tasks that don't have data yet."""
    ...
    for seq_len in args.seq_lengths:
        for task in args.tasks:
            ...
            # MInference-style prep: chat template (llama-3) is baked into
            # `input` at prep time. No --prepare_for_ns (MInference doesn't
            # support it). No answer_prefix is emitted as a separate field —
            # it's concatenated into input via prepare.py's template composition.
            cmd = [
...
def predict_task(model, tokenizer, task, task_config, data_dir, pred_dir, args):
    ...
    with open(pred_file, "a", encoding="utf-8", buffering=1) as fout:
        for sample in tqdm(remaining, desc=f"  {task}"):
            # MInference's inference: plain tokenizer call, no manipulation.
            # The chat template (llama-3 or qwen-3) is already baked into
            # sample["input"] at prep time; there is no separate answer_prefix
            # field in MInference's prepared JSONL. BOS handling differs by
            # family: Llama auto-prepends <|begin_of_text|> (add_bos_token=True
            # default); Qwen3 does not (bos_token=None).
